...
#Function to retrieve a post from the databse
...
def edit(id):
    #get the post from the database with a select query for the post with the id
    post = get_post(id)
    
    
    #determine if the page was requested with GET or POST
     #if POST process the form data. Get data and validate it. Update the post and redirect to the homepage
    if request.method == 'POST':
        #get the title and content
        title = request.form['title']
        content = request.form['content']
...
#Delete page will only be processed with a POST request
